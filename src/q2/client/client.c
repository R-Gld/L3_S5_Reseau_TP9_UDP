...
int main(int argc, char **argv) {
    ...
    if(sendto(sock_fd, message, strlen(message) + 1, 0, (struct sockaddr *)&server, server_size) <= -1) {
        perror("sendto");
        close(sock_fd);
        return EXIT_FAILURE;
    }
    ...
    if(recvfrom(sock_fd, output, MAX_LENGTH, 0, (struct sockaddr *)&server, &server_size) <= -1) {
        perror("recvfrom");
        close(sock_fd);
        return EXIT_FAILURE;
    }
